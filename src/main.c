...
static cairo_surface_t *create_surface(int w, int h)
{
    ...
    surface =
      gdk_window_create_similar_image_surface(gtk_widget_get_window(style_window),
					      CAIRO_FORMAT_ARGB32, w, h, 0);
#else
...
static int get_surface_width(cairo_surface_t *surface)
{
    if (cairo_surface_get_type(surface) == CAIRO_SURFACE_TYPE_IMAGE)
	return cairo_image_surface_get_width(surface);
    else if (cairo_surface_get_type(surface) == CAIRO_SURFACE_TYPE_XLIB)
	return cairo_xlib_surface_get_width(surface);
    else
	return -1;
}

static int get_surface_height(cairo_surface_t *surface)
{
    if (cairo_surface_get_type(surface) == CAIRO_SURFACE_TYPE_IMAGE)
...
static void draw_surface(cairo_surface_t *surface, cairo_operator_t operator,
			 cairo_surface_t *src, int xsrc, int ysrc,
			 int xdest, int ydest, int w, int h, double alpha)
{
    cairo_t *cr = NULL;
    ...
    if (w < 0 || h < 0)
    {
	int src_w, src_h, dest_w, dest_h;

	src_w = get_surface_width(src);
	src_h = get_surface_height(src);
	dest_w = get_surface_width(surface);
	dest_h = get_surface_height(surface);
...
static void draw_shadow_background(decor_t * d, cairo_t * cr)
{
    ...
    if (ws->large_shadow_surface == NULL)
    {
    ...
    width = get_surface_width(ws->large_shadow_surface);
    height = get_surface_height(ws->large_shadow_surface);
					       ...
					       gdouble glow_alpha, int b_state)
{
	...
	if (glow_alpha > 1e-5)	/* i.e. glow is on */
	{
	    ...
	    {
		/* draw glow */
		if (ws->button_glow_surface[b] != NULL)
		{
		    draw_surface(cairo_get_target(cr), CAIRO_OPERATOR_OVER,
	    ...
	    {
		/* draw inactive glow */
		if (ws->button_inactive_glow_surface[b] != NULL)
		{
		    draw_surface(cairo_get_target(cr), CAIRO_OPERATOR_OVER,
...
static void draw_button_backgrounds(decor_t * d, int *necessary_update_type)
{
	...
	if (button_region->bg_surface != NULL)
	    draw_surface(d->buffer_surface != NULL ? d->buffer_surface
			   : d->surface,
			 CAIRO_OPERATOR_SOURCE,
			 button_region->bg_surface, src_x, src_y,
			 dest_x, dest_y, w, h, 1.0);
...
gint draw_buttons_timer_func(gpointer data)
{
    ...
    if (d->buffer_surface == NULL && d->surface == NULL)
    {
	stop_button_fade(d);
	return FALSE;
    }
    ...
    {
	fade_info->cr =
	  cairo_create(d->buffer_surface != NULL ? d->buffer_surface : d->surface);
    ...
    if (d->buffer_surface != NULL && !d->button_fade_info.first_draw &&
	d->min_drawn_buttons_region.x1 < 10000)
    {
...
static void draw_window_decoration_real(decor_t * d, gboolean shadow_time)
{
    ...
    {
	cr = cairo_create(d->buffer_surface != NULL ?
	...
	w = get_surface_width(pbuff);
	h = get_surface_height(pbuff);
...
static void draw_switcher_background(decor_t * d)
{
    ...
    cairo_set_operator(cr, CAIRO_OPERATOR_SOURCE);
    if (d->prop_xid || d->buffer_surface == NULL)
    {
	draw_shadow_background(d, cr);
    }
    ...
    draw_surface(d->surface,
		 CAIRO_OPERATOR_SOURCE,
		 d->buffer_surface, 0, 0, 0, 0, d->width, d->height, 1.0);
...
static void draw_switcher_foreground(decor_t * d)
{
    ...
    if (d->surface == NULL || d->buffer_surface == NULL)
	return;
				       ...
				       frame_settings *fs_inact)
{
    ...
    if (ws->shadow_surface != NULL)
    {
	int width, height;

	width = get_surface_width(ws->shadow_surface);
	height = get_surface_height(ws->shadow_surface);
    ...
    if (ws->decor_normal_surface != NULL)
	cairo_surface_destroy(ws->decor_normal_surface);
    if (ws->decor_active_surface != NULL)
...
static gboolean update_switcher_window(WnckWindow * win, Window selected)
{
    ...
    surface = create_xlib_surface(width, height);
    if (surface == NULL)
	return FALSE;
...
static void action_menu_map(WnckWindow *win, guint button, guint32 time)
{
				 ...
				 guint button, gint bpict)
{
    ...
    {
	case ButtonPress:
	    if (((button == B_T_MAXIMIZE) && xevent->xbutton.button < 3) ||
...
static int update_shadow(frame_settings * fs)
{
    ...
    surface = create_xlib_surface(d.width, d.height);
    if (surface == NULL)
    {
	g_free(params);
	return 0;
    ...
    d.surface = create_xlib_surface(d.width, d.height);
    if (d.surface == NULL)
    {
	g_free(params);
	cairo_surface_destroy(surface);
...
static void load_buttons_image(window_settings * ws, gint y)
{
    ...
    for (i = 0; i < S_COUNT; i++)
    {
	if (ws->button_surface[i + y * S_COUNT] != NULL)
...
static void load_buttons_glow_images(window_settings * ws)
{
	...
	if (ws->button_glow_array == NULL && ws->use_button_glow)
	{
	    /* create an empty surface */
	...
	for (i = 0; i < B_COUNT; i++)
	{
	    if (ws->button_glow_surface[i] != NULL)
	...
	for (i = 0; i < B_COUNT; i++)
	{
	    if (ws->button_inactive_glow_surface[i] != NULL)
